		...
		<section id="section">
			...
			<?php
				...
				if (!$link) {
					...
					$datas = array();
					// sql語法存在變數中
					$sql = "SELECT `SID`,`SName`,`Date`,`boardvalue` FROM `board` INNER JOIN `student` ON `board`.`SID`=`student`.`ID`;";
					...
					echo'<td><font size ="4" face"標楷體"color = white>留言日期</td>';
					echo'<td><font size ="4" face"標楷體"color = white>留言內容</td>';
					// 如果有資料
					if ($result) {
						// mysqli_num_rows方法可以回傳我們結果總共有幾筆資料
						if (mysqli_num_rows($result)>0) {
