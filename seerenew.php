...
<!-- End Login Page Nav -->
  ...
  <main id="main" class="main">
    ...
    <div class="pagetitle">
      ...
      <nav>
        <ol class="breadcrumb">
          <li class="breadcrumb-item"><a href="index.html">宿舍申請</a></li>
          <li class="breadcrumb-item active">房間修改</li>
        </ol>
      </nav>
    </div><!-- End Page Title -->
          ...
          <div class="card">
            <div class="card-body pt-3">
              ...
              <ul class="nav nav-tabs nav-tabs-bordered">
                ...
                <li class="nav-item">
                  <button class="nav-link active" data-bs-toggle="tab" data-bs-target="#profile-overview">房間申請</button>
                </li>
              </ul>
			...
			$mysql_database='dormitoriesdb'; //改成自己的mysql資料庫名
			$conn=mysqli_connect($mysql_server_name,$mysql_username,$mysql_password,$mysql_database) or die("error connecting") ; //連線資料庫
			$sql ="SELECT `identity`,`SName` FROM `identity` INNER JOIN `student` ON `student`.`Idnum`=`identity`.`Idnumber` WHERE	`ID`='$account';"; //SQL語句
			$result = mysqli_query($conn,$sql); //查詢
			while($row = mysqli_fetch_array($result))
			{
				$sid = $row['SID'];
			...
			<tr>
			<td>房號:</td><td><input type="text" name='roomnum' value=<?php echo  $roomnum?>><br/></td>
			</tr>
			<tr>
			<td>棟別:</td><td><input type="text" name='roomnum' value=<?php echo  $bname?>><br/></td>
			</tr>
			<tr>
