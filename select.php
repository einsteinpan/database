		...
		<section id="section">
			...
			<?php
				...
				if (!$link) {
					...
					$datas = array();
					// sql語法存在變數中
					$sql = "SELECT `ID`,`SName`,`Phonenumber`,`Email`,`bname`,`roomnum` FROM `student` INNER JOIN `account` ON `student`.`ID`=`account`.`SID` WHERE	`ID`='$account';";
					...
					if ($result) {
						...
						if (mysqli_num_rows($result)>0) {
							...
							while ($row = mysqli_fetch_array($result)) {
								...
								echo'<td><font size ="4" face"標楷體"color = white>'.$row[4].'</td>';
								echo'</tr>';
								echo'<tr>';
								// 每跑一次迴圈就抓一筆值，最後放進data陣列中
								echo'<td><font size ="4" face"標楷體"color = white>房號</td>';
								echo'<td><font size ="4" face"標楷體"color = white>'.$row[5].'</td>';
								echo'</tr>';
							}
						}
						// 釋放資料庫查到的記憶體
