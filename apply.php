...
<div class="card-body">
    ...
    <form id="L" method="post" >
			...
			<td><select name="bname">
					<option value="學一宿舍">學一宿舍:7550</option>
					<option value="學二宿舍">學二宿舍:9800</option>
					<option value="綜合宿舍">綜合宿舍:7250</option>
				</select></td>
			<td><input type="submit" name="submit" value= "送出" onClick="return showmessage()"></td>
			</tr>
			</form>
			<?php
			$mysql_server_name='localhost'; //改成自己的mysql資料庫伺服器
			...
			$mysql_database='dormitoriesdb'; //改成自己的mysql資料庫名
			$conn=mysqli_connect($mysql_server_name,$mysql_username,$mysql_password,$mysql_database) or die("error connecting") ; //連線資料庫
			$sql = "SELECT * FROM `appinformation` INNER JOIN `account` ON `appinformation`.`SID`=`account`.`account` WHERE account='$account';";
			$result = mysqli_query($conn,$sql); //查詢
			while($row = mysqli_fetch_array($result))
			{
				$id=$row['SID'];
			...
			if(isset($_POST['bname'])) {
				...
				$bname = $_POST['bname'];
				$sql="update `appinformation` set `bname`= '"."$bname' where `SID`='$id';";
				$result = mysqli_query($conn,$sql); //查詢
				$sql="update `student` set `bname`= '已申請' where `ID`='$id';";
				$result = mysqli_query($conn,$sql); //查詢
				$sql="update `student` set `roomnum`= '待定' where `ID`='$id';";
				$result = mysqli_query($conn,$sql); //查詢
